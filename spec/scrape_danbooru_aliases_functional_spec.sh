...
Describe "scrape_danbooru_aliases.zsh comprehensive functionality"
  ...
  Context "Error handling"
    ...
    It "should fail when curl is missing"
      When run zsh -c 'PATH="/dev/null" "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh"'
      The stderr should include "curl is required but not installed"
      The status should be failure
    End

    It "should fail when jq is missing"
      When run zsh -c 'PATH="/usr/bin:/bin" "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh"'
  ...
  Context "JSON processing functions"
    It "should process valid JSON arrays correctly"
      ...
      When call zsh -c "source "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh" && json_to_csv '$json'"
    ...
    It "should reject invalid JSON"
      ...
      When call zsh -c "source "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh" && json_to_csv '$invalid_json'"
  ...
  Context "API request functionality"
    It "should make proper API requests"
      When call zsh -c "source "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh" && api_request 1"
    ...
    It "should handle empty API responses"
      When call zsh -c "source "$SHELLSPEC_PROJECT_ROOT/scrape_danbooru_aliases.zsh" && api_request 3"
